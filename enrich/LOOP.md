...
5. **Craft edit (separate editor, makes it BETTER).** Spawn a SEPARATE `Task` subagent as a line editor. Its only job: rewrite the story to hit `enrich/VOICE.md`'s "Make it sing" bar, a strong in-scene open, one earned line at the turn, varied rhythm, NO résumé/list cadence (never "worked at X, then Y, then Z"), and it must FIND the one through-line that's truly this person's (the descended name, the repeated trade, the place they returned to) and END on it. It may use facts already true elsewhere in the tree for that closing line, but must NOT invent or alter any fact, name, date, or place. Apply its version. (Writer drafts, editor makes it sing.)
